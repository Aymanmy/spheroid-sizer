"""
 Spheroid Sizer - Detection and Reanalysis
==============================================================
- Better automatic detection to avoid small round shapes
- Live contour preview during detection
- Single file and folder processing options
- Hybrid detection around manual points
- Real-time parameter adjustment
- Reanalysis table with thumbnails and selection
- Save processed images with contours
- Excel export with multiple sheets (one per folder)
- Automatic preview after processing each folder
"""
...
import cv2
import re
from PIL import Image, ImageTk, ImageDraw, ImageOps  # Added ImageOps
...
class SpheroidSizerApp:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Enhanced Spheroid Sizer v5.2 - With Reanalysis Table")
        self.setup_gui()
    ...
    def setup_gui(self):
        ...
        tk.Label(title_frame, text="Enhanced Spheroid Sizer v5.2", 
...
✓ Automatic preview after processing each folder"""
        ...
        tk.Button(btn_frame, text="Process Multiple Folders", command=self.process_multiple_folders,
                 font=('Arial', 12), bg='#9b59b6', fg='white', padx=20, pady=10).pack(side=tk.LEFT, padx=10)
...
• Support for processing multiple folders in batch mode"""
    ...
    def open_donate_link(self):
        ...
        donate_url = "https://paypal.me/AGSpheroidsizer"  # Replace with your link
...
• Size Category: {'Large' if area_um2 > 10000 else 'Medium' if area_um2 > 2000 else 'Small'}"""
    ...
    def process_multiple_folders(self):
        """Process multiple folders in batch mode"""
        # Try file dialog first, with fallback to manual input
        parent_folder = None
        
        try:
            parent_folder = filedialog.askdirectory(title="Select Parent Folder Containing Subfolders")
        except Exception as e:
            logger.warning(f"File dialog failed: {e}")
        
        # If file dialog failed or user cancelled, ask for manual input
        if not parent_folder:
            parent_folder = simpledialog.askstring(
                "Parent Folder Path", 
                "Enter the full path to your parent folder:\n(e.g., /home/runner/MyProject)",
                parent=self.root
            )
        ...
        if not os.path.exists(parent_folder):
            messagebox.showerror("Error", f"Folder does not exist: {parent_folder}")
            return
        
        if not os.path.isdir(parent_folder):
            messagebox.showerror("Error", f"Path is not a directory: {parent_folder}")
            return
        ...
        # Get common scale for all folders
        scale = simpledialog.askfloat("Scale Factor", "Enter scale (micrometers per pixel) for all folders:", 
                                     parent=self.root, minvalue=0.001)
        if scale is None:
            return
        ...
        for folder in subfolders:
            try:
                self.status_var.set(f"Processing folder: {os.path.basename(folder)}")
                self.root.update()
                
                results_df = self._process_folder_batch(folder, scale)
                if results_df is not None and not results_df.empty:
        ...
        if not all_results.empty:
            try:
                ...
                csv_path = os.path.join(parent_folder, f"combined_results_{timestamp}.csv")
                all_results.to_csv(csv_path, index=False)
                messagebox.showinfo("Batch Complete", 
                                  f"Processed {len(subfolders)} folders\nResults saved to:\n{csv_path}")
            except Exception as e:
...
def main():
    """Main entry point"""
    logger.info("Starting Spheroid Sizer")
